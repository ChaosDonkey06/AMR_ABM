...
def model(patients_state, gamma, beta, alpha_ens, movement_date, ward2size, ward2cluster=None, se=4/100):
    """_summary_

    Args:
        patients_state (_type_):                  _description_
        gamma (_type_):                           _description_
        beta (_type_):                            _description_
        alpha_ens (_type_):                       _description_
        movement_date (_type_):                   _description_
        ward2size (_type_):                     _description_
        ward_observation_dict (_type_, optional): _description_. Defaults to None.
        se (_type_, optional):                    Likelihood of positive | test and positive . Defaults to 4/100.
    ...
    for _, ward_id in enumerate(active_wards):
        ...
        foi_ward                          = beta_ens  * colonized_patients / ward2size[ward_id] # Ward force of infection.
        foi                               = foi_ward
...
def model_inference(patients_state, gamma, beta, alpha_ens, movement_date, ward2size, ward2cluster, se=6/100):
    """[summary]
    ...
    num_ward_chunks       = len(set(list(ward2cluster.values())))
    ...
    return patients_state, ward_positive, ward_chunk_positive, ward_negative, ward_chunk_negative


def simulate_model(movement_data, ward2size, ward2community, params_dict, abm_settings, model=model):
    """ Simulate model using point parameters in param_dict.

    Args:
        model (_type_): _description_
        movement_data (_type_): _description_
        ward2size (_type_): _description_
        ward2community (_type_): _description_
        params_dict (_type_): _description_
        abm_settings (_type_): _description_
    ...
    gamma_truth  = params_dict["gamma"]
    beta_truth   = params_dict["beta"]
    ...
    ward_chunk_positive           = np.full((len(abm_settings["dates"] ),  abm_settings["num_wards_chunks"], abm_settings["num_ensembles"]), np.nan)
    ward_chunk_colonized          = np.full((len(abm_settings["dates"] ),  abm_settings["num_wards_chunks"], abm_settings["num_ensembles"]), np.nan)
    ward_chunk_nosocomial         = np.full((len(abm_settings["dates"] ),  abm_settings["num_wards_chunks"], abm_settings["num_ensembles"]), np.nan)
    ward_chunk_colonized_imported = np.full((len(abm_settings["dates"] ),  abm_settings["num_wards_chunks"], abm_settings["num_ensembles"]), np.nan)

    ward_negative                 = np.full((len(abm_settings["dates"] ),  abm_settings["num_wards"], abm_settings["num_ensembles"]), np.nan)
    ward_chunk_negative           = np.full((len(abm_settings["dates"] ),  abm_settings["num_wards_chunks"], abm_settings["num_ensembles"]), np.nan)
    ...
    for i_d, date in tqdm(enumerate(list(abm_settings["dates"] ))):
        ...
        patients_state, ward_colonized[i_d,:], ward_nosocomial[i_d,:], ward_colonized_imported[i_d,:], ward_positive[i_d,:], ward_chunk_colonized[i_d,:], \
            ward_chunk_nosocomial[i_d], ward_chunk_colonized_imported[i_d,:], ward_chunk_positive[i_d,:], ward_negative[i_d, :], ward_chunk_negative[i_d,:] = model(patients_state, gamma_ens, beta_ens, alpha_ens, movement_date, ward2size, ward2community)

    return ward_colonized, ward_nosocomial, ward_colonized_imported, ward_positive, ward_chunk_colonized, ward_chunk_nosocomial, ward_chunk_colonized_imported, ward_chunk_positive, ward_negative, ward_chunk_negative
